...
import { fetchMovieDetails, fetchMovieCredits, fetchRecommendations, fetchSimilar, getImageUrl } from '../services/tmdb';
import MovieCard from '../components/MovieCard';
import Loader from '../components/Loader';
import { motion, AnimatePresence } from 'framer-motion';
import { FaStar, FaClock, FaCalendar, FaHeart, FaRegHeart, FaListUl, FaCheck, FaEye, FaRegEye, FaArrowLeft, FaArrowRight, FaTimes, FaSignInAlt } from 'react-icons/fa';
...
const MovieDetails = () => {
    ...
    useEffect(() => {
        const load = async () => {
            ...
            try {
                ...
                let related = await fetchRecommendations(id, lang);
                if (!related || related.length < 4) {
                    const similar = await fetchSimilar(id, lang);
                    const ids = new Set(related.map(m => m.id));
                    for (const m of similar) {
                        if (!ids.has(m.id) && related.length < 4) {
                            related.push(m);
                            ids.add(m.id);
                        }
                    }
    ...
    const handleAction = (actionFn, movie) => {
        if (!user) {
        ...
        <div>
                ...
                <div className="container" style={{ padding: '2rem 1.5rem', position: 'relative', zIndex: 1, display: 'flex', gap: '2rem', flexWrap: 'wrap', alignItems: 'flex-end' }}>
                    ...
                    <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.15 }} style={{ flex: 1, minWidth: '250px' }}>
                        ...
                        <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
                            <button onClick={() => handleAction(toggleWatchlist, movie)} className="detail-action-btn" style={{ background: isInWatchlist(movie.id) ? 'var(--primary-color)' : 'rgba(255,255,255,0.08)' }}>
                                {isInWatchlist(movie.id) ? <FaCheck /> : <FaListUl />} {isInWatchlist(movie.id) ? t('added_watchlist') : t('add_watchlist')}
                            </button>
                            <button onClick={() => handleAction(toggleWatched, movie)} className="detail-action-btn" style={{ background: isWatched(movie.id) ? 'rgba(16,185,129,0.9)' : 'rgba(255,255,255,0.08)' }}>
                                {isWatched(movie.id) ? <FaEye /> : <FaRegEye />} {lang === 'ar' ? 'تم المشاهدة' : 'Watched'}
                            </button>
                            <button onClick={() => handleAction(toggleFavorite, movie)} className="detail-action-btn" style={{ background: isInFavorites(movie.id) ? 'var(--accent-color)' : 'rgba(255,255,255,0.08)' }}>
            ...
            <div className="container" style={{ padding: '2.5rem 1.5rem' }}>
                ...
                {credits?.actors?.length > 0 && (
                    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} style={{ marginBottom: '3rem' }}>
                ...
                {relatedMovies.length > 0 && (
                    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }}>
                        <h2 style={{ marginBottom: '1.5rem', fontSize: '1.4rem' }}>{t('related_movies')}</h2>
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: '1.5rem', maxWidth: '900px' }}>
                            {relatedMovies.map(m => (
                                <motion.div key={m.id} whileHover={{ y: -5 }} transition={{ duration: 0.2 }}>
                                    <MovieCard movie={m} />
                                </motion.div>
                            ))}
                        </div>
